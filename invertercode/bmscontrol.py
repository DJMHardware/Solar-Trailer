...
class handle_rs232(threading.Thread):
    ...
    def decode(self):
        ...
        if re.match(r'^:[A-F0-9]+', self.reply):
            ...
            self.reply_command = int(self.reply[5] + self.reply[6]
                                     + self.reply[7] + self.reply[8], 16)
            if ((self.value['Bytes'] * self.value['Encode']) == self.reply_len
                    and (self.key == self.reply_command)):
